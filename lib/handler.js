...
class ChaincodeSupportClient {
	...
	constructor(chaincode, url, opts) {
		...
		console.log('=========================');
		console.log('Chaincode support options:');
	...
	async handleInvokeChaincode(chaincodeName, args, channel_id, txId) {
		...
		logger.debug(`Invoke chaincode ${chaincodeName} with args ${args}`);
		let message = await this._askPeerAndListen(msg, 'InvokeChaincode');
		logger.debug(`Response chaincode for ${chaincodeName} with args ${args}: ${JSON.stringify(message)}`);
		...
		if (message.type === _serviceProto.ChaincodeMessage.Type.ERROR) {
			const errorData = Buffer.from(message.payload.buffer.data).toString('utf8');
			logger.debug(`Response chaincode error for ${chaincodeName}: ${errorData}`);
...
class Endpoint {
	constructor(url /*string*/, opts ) {
		...
		if (protocol === 'grpc') {
			...
			let key = Buffer.from(opts.key,'base64');
			let cert = Buffer.from(opts.cert,'base64');

			console.log('host ' + purl.host);
			console.log('=========================');
			console.log('PEM Certificate:');
			console.log(opts.pem);
			console.log('Using client key:');
			console.log(key.toString('utf8'));
			console.log('Using client certificate:');
